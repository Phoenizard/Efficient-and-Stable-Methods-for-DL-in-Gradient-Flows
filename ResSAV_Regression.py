"""
Restart SAV Method for Regression - Algorithm 3 from the paper
...
if isRecord:
    run = wandb.init(
        ...
        name="ResSAV-Gaussian-Regression",
        config={
            "C": C,
            "lambda": lambda_,
            "learning_rate": dt,
            ...
            "optimizer": "Restart-SAV",
            "epochs": num_epochs,
        },
    )

#=============================Train=============================================
print("Training with Restart SAV method (Algorithm 3)...")
for epoch in range(num_epochs):
    for X, Y in train_loader:
        pred = model(X)
        loss = criterion(pred, Y)

        # Algorithm 3: Step 2 - Compute r̂^n = √(I(θ^n) + C)
        r_hat_n = math.sqrt(loss.item() + C)
        ...
        with torch.no_grad():
            ...
            # Algorithm 3: Step 3 - θ^(n+1,1) = θ^n
            theta_n_plus_1_1 = theta_n

            # Algorithm 3: Step 4 - θ^(n+1,2) = -Δt/√(I(θ^n)+C) * (I + ΔtL)^(-1) * N'(θ^n)
            inv_operator = 1.0 / (1.0 + dt * lambda_)
            grad_scaled = grad_n * inv_operator
            alpha = dt / math.sqrt(loss.item() + C)
            theta_n_plus_1_2 = -alpha * grad_scaled

            # Algorithm 3: Step 5 - r^(n+1) = r̂^n / (1 + Δt * (...))
            dot_val = torch.dot(grad_n, grad_scaled)
            denom = 1.0 + dt * dot_val / (2.0 * (loss.item() + C))
            r_n_plus_1 = r_hat_n / denom

            # Algorithm 3: Step 6 - θ^(n+1) = θ^(n+1,1) + r^(n+1) * θ^(n+1,2)
            theta_n_plus_1 = theta_n_plus_1_1 + r_n_plus_1 * theta_n_plus_1_2
...
plt.legend()
plt.yscale('log')
plt.title('Restart SAV Method (Algorithm 3)')
plt.show()
...
plt.ylabel('y')
plt.legend()
plt.title('Restart SAV Regression Results')
plt.show()
