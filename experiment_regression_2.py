"""
Experiment: Regression Task 2
Target function: f*(x) = Σ c_i x_i^2 (Quadratic function)

This experiment compares different optimization algorithms for a quadratic regression problem.
"""
...
os.makedirs('results/experiment_2', exist_ok=True)

print("="*60)
print("Experiment: Regression Task 2 (Quadratic Function)")
print("="*60)

# Load data
print("\nLoading data...")
(x_train, y_train) = torch.load('data/Example2_train_data.pt')
(x_test, y_test) = torch.load('data/Example2_test_data.pt')
...
hist_sav = sav_regression(x_train, y_train, x_test, y_test,
                          m=m, batch_size=64, C=1, lambda_=4, dt=0.4,
                          num_epochs=num_epochs, device=device)
...
hist_esav = esav_regression(x_train, y_train, x_test, y_test,
                            m=m, batch_size=64, C=1, lambda_=1, dt=0.1,
                            num_epochs=num_epochs, device=device)
...
print("="*60)
torch.save(results, 'results/experiment_2/results.pt')
print("Results saved to: results/experiment_2/results.pt")
...
ax1.set_yscale('log')
ax1.set_title('Training Loss Comparison (Quadratic Regression)')
ax1.legend()
...
ax2.set_yscale('log')
ax2.set_title('Test Loss Comparison (Quadratic Regression)')
ax2.legend()
ax2.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig('results/experiment_2/loss_comparison.png', dpi=300, bbox_inches='tight')
print("Loss comparison plot saved to: results/experiment_2/loss_comparison.png")
